...
%% today's date: Y-m-D H:i:s
get_date() ->
  {{Y, M, D},{H, I, S}} = erlang:localtime(),
  list_to_binary([integer_to_list(Y),"-", integer_to_list(M), "-", integer_to_list(D), " ", integer_to_list(H), ":", integer_to_list(I), ":", integer_to_list(S)]).
get_date(Timestamp) ->
  {{Y, M, D},{H, I, S}} = seconds_to_localtime(Timestamp),
  list_to_binary([integer_to_list(Y),"-", integer_to_list(M), "-", integer_to_list(D), " ", integer_to_list(H), ":", integer_to_list(I), ":", integer_to_list(S)]).

printf(Data) ->
  io:format("~n ~p ~n", [Data]).
printf(Flag, Data) ->
  io:format("~n [~p]~p ~n", [Flag, Data]).

mark(Flag, String) ->
  error_logger:info_report(io_lib:format("[~p]~p", [Flag, String])).

elog(Flag, String) ->
  error_logger:format("[~p]~p", [Flag, String]).

trinomial(Bool, TrueProceed, FalseProceed) ->
  case Bool of
    true ->
      case is_function(TrueProceed) of
        true  -> apply(TrueProceed, []);
        false -> TrueProceed
      end;
    false ->
      case is_function(FalseProceed) of
        true  -> apply(FalseProceed, []);
        false -> FalseProceed
      end
  end.
      ...
      case length(L) > 0 of
        true->
          F = fun(X, S)->
            E1 = element(1,X),
            E2 = element(2,X),
            case is_integer(E1) of
              true -> Estr1 = integer_to_list(E1);
              false-> Estr1 = E1
            end,
            case is_integer(E2) of
              true -> Estr2 = integer_to_list(E2);
              false-> Estr2 = E2
            end,
            Tmp1 = string:concat(string:concat("{\"",Estr1),"\":\""),
            Tmp2 = string:concat(Estr2,"\"},"),
            Tmp  = string:concat(Tmp1,Tmp2),
            string:concat(S, Tmp)
              end,
          S1 = lists:foldl(F,"[", L),
          string:concat(string:substr(S1,1,string:len(S1) - 1),"]");
        false->
          []
      end;
    false->
